...
The original Phase 2c plan is one monster session. Sliced into 5 self-contained phases so each can run as a fresh Claude session with a single commit + PR between them. Each phase ships independently, has a narrow review surface, and doesn't block the next one from starting cold.

**Scope cuts from original plan:**
- Wave H (insulation) → separate plan, not here.
- Wave G (verify) → folded into each phase's Verify section, not a phase.

**Dependency chain:** P0 anytime → P1 → P2 → P3 → P4. Skipping is fine; reordering P1→P3 is not.

**Branch strategy:** each phase pushes to its own branch off `main`. This planning branch (`claude/gallery-v2-planning-9dNRJ`) stays for the plan doc + vault durability only.
...
**Why now:** lock the plan + post-merge todos into the repo before any code work starts.
