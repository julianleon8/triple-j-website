...
Shipped: source registry rebuilt (`temple` on, `bell_county` off, `pathFilter`/`exclude` against the full path, `PDF_HREF_PATTERN` now captures the `?t=` upload stamp as the recency signal). New pure module `src/lib/jobs/scrape-permits.ts` — base-href resolution, stamp ordering, unseen picking, row splitting, job-type pre-filter, stall rule, digest copy — with a trimmed HTML fixture, a synthetic-name report fixture, and 33 tests. Extractor takes pre-filtered rows, batches 40 per Claude call, prompt-cached, and returns owner, applicant, contractor, municipal status, code and a `lead_class` (accessory / new_home / commercial). Route reads up to three unseen reports per run, inserts new permits, updates only the municipal status on re-sightings, records `permit_reports`, and pushes a per-run digest, a hot-permit alert, and a Monday-only stall nag after 14 quiet days. `cronRoute` now hands the job the request so a manual POST can carry `{ maxReports }`. HQ table gained class filters, Owner / Contractor / Status columns and row anchors; the calendar's link to a per-permit page that never existed now targets the table row; the testing panel reads `detail.summary`. Migration 031 applied to production: six columns, full `unique (jurisdiction, permit_number)`, `permit_reports`.

Validation: typecheck, lint, 254 tests across 21 files pass. First production run: see the primer.
