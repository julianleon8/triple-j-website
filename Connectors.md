...
| `scrape-permits` | `0 14 * * *` | Temple weekly permit PDFs → `permit_leads` + `permit_reports`. Reads up to 3 unseen reports per run. Manual `POST` (may carry `{ maxReports }`) answers 202 and runs detached; `GET …/scrape-permits/status` (owner) returns the latest `cron_runs` row for the page to poll |
