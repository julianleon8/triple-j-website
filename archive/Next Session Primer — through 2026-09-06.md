# Next Session Primer — handoffs through 2026-09-06

Split out of `Next Session Primer.md` on 2026-09-07, when that file had grown to 25 blocks
reaching back to 2026-04-21 and no longer answered the one question it exists to answer:
where did the last session stop. Nothing was deleted — every block below is verbatim.

**Nothing here is authoritative.** These are dated handoffs describing what was true when they
were written. For what is true now read `Locked Decisions.md`; for session history read
`Session Notes.md`.
